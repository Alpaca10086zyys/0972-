...
def load_dataset(name="dino"):
    if name == "dino":
        ...
        df = pd.read_csv(f"seed_datasets/{name}.csv", header=None, names=['x','y'])
    return df.copy()
...
def run_pattern(df, target, iters = 100000, num_frames=100, decimals=2, shake=0.2,
    ...
    for i in looper(iters+1, leave=True, ascii=True, desc=target + " pattern"):
        ...
        # 要创建的目录路径
        dir_path = f'results/{shape_start}-{target}/iters={iters}'
        # 检查目录是否存在
        ...
        for x in range(write_frames.count(i)):
            save_scatter_and_results(r_good, f"results/{shape_start}-{target}/iters={iters}/" + "image-"+format(int(frame_count), '05'), 150, labels = labels)
            #save_scatter(r_good, target + "-image-"+format(int(frame_count), '05'), 150)
            r_good.to_csv(f"results/{shape_start}-{target}/iters={iters}/" + "data-" + format(int(frame_count), '05') + ".csv")
